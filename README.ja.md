...
変換は `bdf generate` サブコマンドで行い、入力の形式（PDF / PowerPoint / Excel / Windows メタファイル）は中身から判別します。
...
# PDF / PowerPoint / Excel / メタファイル → BDF（形式は中身から判別。-format pdf|pptx|xlsx|emf で指定も可）
