...
| `source` | 変換元の形式（`pdf` / `pptx` / `xlsx` / `vsdx` / `vdx` / `emf` / `wmf` / `tiff` / `fixture` …）。Dublin Core の `source` とは別物 |
...
| 要素 | PDF（文書情報辞書） | PowerPoint、Excel、Word（コアプロパティ） | TIFF（先頭のページのタグ） |
|---|---|---|---|
...
| `modified` | `ModDate`（W3CDTF に変換） | `dcterms:modified` | `DateTime`（W3CDTF に変換） |
