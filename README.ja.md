...
- manifest に Dublin Core のメタデータ（題名・作成者・主題・言語・作成日時など）を持てる。PDF の文書情報、PowerPoint のコアプロパティ、Visio の文書プロパティから引き継ぐ

変換は `bdf generate` サブコマンドで行い、入力の形式（PDF / PowerPoint / Visio / Windows メタファイル）は中身から判別します。
...
# PDF / PowerPoint / Visio / メタファイル → BDF（形式は中身から判別。-format pdf|pptx|visio|emf で指定も可）
