...
// the slide (bg1 → lt1 …) and the placeholder color of style references.
...
func (cc *colorCtx) color(n *node) (rgba, bool) {
	if n == nil {
		return rgba{}, false
	}
...
func (cc *colorCtx) resolve(n *node) (rgba, bool) {
	var c rgba
	ok := true
	switch n.Name {
	case "srgbClr":
	...
	case "scrgbClr":
		c = rgba{fromLinear(pct(n, "r", 0)), fromLinear(pct(n, "g", 0)), fromLinear(pct(n, "b", 0)), 1}
	case "hslClr":
		c = hslToRGB(float64(n.attrInt("hue", 0))/60000/360, pct(n, "sat", 0), pct(n, "lum", 0))
		c.A = 1
	default:
		return rgba{}, false
	}
...
func applyMods(c rgba, n *node) rgba {
