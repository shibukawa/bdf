...
type chartCtx struct {
	...
	root       *ooxml.Node
	x, y, w, h float64
	base       *ooxml.Node // chart-wide default run properties
	nseries    int
}

func (s *Drawing) drawChart(cv *canvas.Canvas, sh *shape, xf xform, rid string) {
...
func (ch *chartCtx) draw() {
	...
	if t := chart.Child("title"); t != nil && !autoDeleted {
		...
		top += ch.title(&ooxml.Node{Name: "title"}, plots, top)
	}
	// legend
...
func (s *Drawing) boxFill(cv *canvas.Canvas, spPr *ooxml.Node, part string, x, y, w, h float64) {
	if spPr == nil {
		return
	}
	...
	if l := spPr.Child("ln"); l != nil {
		ln = s.resolveLine([]*ooxml.Node{l}, []string{part}, nil, s.cc)
	}
...
func (ch *chartCtx) parsePlot(k *ooxml.Node) *chartPlot {
	...
	for _, sn := range k.Children("ser") {
		se := &chartSeries{idx: ch.nseries, spPr: sn.Child("spPr"), dPts: map[int]*ooxml.Node{}, marker: sn.Child("marker"), dLbls: sn.Child("dLbls")}
...
func cacheNumbers(n *ooxml.Node, format *string) []float64 {
	...
	if format != nil {
		*format = cache.Child("formatCode").Content()
	}
	count := int(cache.Child("ptCount").AttrInt("val", 0))
	pts := cache.Children("pt")
	for _, pt := range pts {
	...
	for _, pt := range pts {
		if i := int(pt.AttrInt("idx", -1)); i >= 0 && i < count {
			if v, err := strconv.ParseFloat(strings.TrimSpace(pt.Child("v").Content()), 64); err == nil {
				out[i] = v
			}
		}
	}
...
func (ch *chartCtx) textStyle(txPr *ooxml.Node, size float64, bold bool) *runStyle {
	// size is the element's default, relative to the chart's base size
	def := mkNode("defRPr", "sz", itoa(int(math.Round(size*100))))
	if bold {
		def.Attrs = append(def.Attrs, mkNode("", "b", "1").Attrs...)
	}
...
func (ch *chartCtx) measure(st *runStyle, text string) float64 {
	w := 0.0
	for _, r := range text {
		w += ch.s.c.fonts.Advance(ch.s.c.faceFor(st, r), r)*st.size + st.spacing
	}
	return w
}
...
func (ch *chartCtx) label(st *runStyle, text string, x, y float64, ha, va byte) {
	...
	em := &textEmitter{c: ch.s.c, cv: ch.cv}
	ch.cv.Obj.Save()
...
func (ch *chartCtx) title(t *ooxml.Node, plots []*chartPlot, top float64) float64 {
	if rich := t.Path("tx", "rich"); rich != nil {
		...
		tf.extra = []*ooxml.Node{def, ch.base}
		// center the paragraphs
		ctr := mkNode("defPPr", "algn", "ctr")
		tf.lists = []*ooxml.Node{&ooxml.Node{Name: "lstStyle", Kids: []*ooxml.Node{ctr}}}
		b := ch.s.layoutText(tf, ch.w*0.1, top, ch.w*0.8, ch.h, canvas.Identity)
		if b == nil {
			return 0
		}
		b.h = b.height()
...
func (ch *chartCtx) legend(lg *ooxml.Node, plots []*chartPlot, top, bottom, left, right float64) (float64, float64, float64, float64) {
	...
	draw := func(e legendEntry, x, y float64) {
		...
		ch.cv.Obj.Save()
		if ch.s.setFill(ch.cv, e.f, key, key) {
...
func (ch *chartCtx) marker(se *chartSeries, f fill, x, y float64, symbol string, size float64) {
	...
	} else if ch.s.setFill(ch.cv, f, size, size) {
...
func niceAxis(lo, hi float64, n *ooxml.Node, maxSteps int) *axis {
	a := &axis{node: n}
...
func (ch *chartCtx) axesChart(pa *ooxml.Node, plots []*chartPlot, px, py, pw, ph float64, inner bool) {
	s, cv := ch.s, ch.cv
	p0 := plots[0]
	...
	var valAxNode, catAxNode *ooxml.Node
	for _, k := range pa.Kids {
		switch k.Name {
		case "valAx":
			if scatter {
	...
	// gridlines
	gridLine := func(ax *ooxml.Node, name string) *line {
		g := ax.Child(name)
		if g == nil {
			return nil
		}
		if ln := g.Path("spPr", "ln"); ln != nil {
			return s.resolveLine([]*ooxml.Node{ln}, []string{ch.part}, nil, s.cc)
		}
	...
	// axis lines
	axisLine := func(ax *ooxml.Node) *line {
...
func catLabel(s string, ax *ooxml.Node) string {
...
func (ch *chartCtx) pie(p *chartPlot, x, y, w, h float64) {
	...
	for i, v := range se.vals {
		...
		cv.Obj.Save()
		if ch.s.setFill(cv, f, 2*r, 2*r) {
			cv.Obj.FillPath(ref, 0)
		}
		if l != nil {
			setLine(cv, l, 2*r, 2*r)
...
func dLblsFor(p *chartPlot, se *chartSeries) *ooxml.Node {
	if se.dLbls != nil {
		return se.dLbls
	}
	return p.dLbls
...
func showFlag(d *ooxml.Node, name string) bool {
