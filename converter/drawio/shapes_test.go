...
func TestWaypoints(t *testing.T) {
	got := getWaypoints([]point{{0, 0}, {0.5, 0}, {10, 0}, {10, 0.2}, {10, 10}})
	want := []point{{0, 0}, {10, 0}, {10, 10}}
	if !slices.Equal(got, want) {
		t.Errorf("getWaypoints = %v, want %v", got, want)
	}
	if got := getWaypoints([]point{{0, 0}, {0.5, 0.5}}); got != nil {
		t.Errorf("an edge shorter than a unit is not painted: %v", got)
	}
}
