...
## 3.13 画像 → BDF 変換器（converter/image）の構造
...
11. **画像**: ブラウザが表示できる画像はそのまま格納し、メタデータだけを読む。SVG はページが描く（実装済み、§3.13）。
...
│                      linebreak（行分割の規則）、CAD の変換器で共有する cad（図面をページに描く）
...
├── testdata/          Go が生成した demo.bdf / demo-split / demo-encrypted.bdf、PDF・PowerPoint・Excel・Visio・Word・DXF・画像などの変換結果と golden PNG
