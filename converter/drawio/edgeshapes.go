...
// waypoints returns the points of an edge without those less than a unit
// away from the point before (mxShape.getWaypoints at scale 1), which edge
// shapes need to find their directions.
func waypoints(pts []point) []point {
	if len(pts) == 0 {
		return pts
	}
	out := []point{pts[0]}
