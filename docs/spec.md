...
| `source` | 変換元の形式（`pdf` / `pptx` / `xlsx` / `vsdx` / `vdx` / `emf` / `wmf` / `fixture` …）。Dublin Core の `source` とは別物 |
...
`LINK` の矩形は他の命令と同じく現在の変換行列の座標系で表す。読み手は 4 隅を変換した外接矩形をリンク領域とし、その領域に重なる run をリンクの文字列とする。`url` は `http:` / `https:` / `mailto:` の絶対 URL か、同じ View のページを指す `#page=N`（1 始まり）。読み手はそれ以外の `url` をリンクにしない。
