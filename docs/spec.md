...
| `source` | 変換元の形式（`pdf` / `pptx` / `xlsx` / `csv` / `vsdx` / `vdx` / `drawio` / `dxf` / `emf` / `wmf` / `png` / `jpeg` / `gif` / `webp` / `avif` / `bmp` / `ico` / `svg` / `fixture` …）。Dublin Core の `source` とは別物 |
...
PDF の対応は XMP が文書情報辞書を写す方法に合わせている。
