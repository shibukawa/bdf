...
- 読み上げ可能なテキスト層: 構造 MARK の見出し・リスト・表・代替テキスト付きの図・リンク・言語をスクリーンリーダーに伝える（タグ付き PDF と PowerPoint の構造、Excel と CSV のセルと表の見出しを変換）
- 1 ファイル形式と分割ファイル形式を相互変換可能（1 ファイル形式はマジック `bdf\0` で始まる）
- manifest に Dublin Core のメタデータ（題名・作成者・主題・言語・作成日時など）を持てる。PDF の文書情報、PowerPoint・Excel のコアプロパティ、Visio の文書プロパティから引き継ぐ
...
変換は `bdf generate` サブコマンドで行い、入力の形式（PDF / PowerPoint / Excel / CSV / Visio / Windows メタファイル）は中身から、判別できなければ拡張子から決めます。
...
# PDF / PowerPoint / Excel / CSV / Visio / メタファイル → BDF（形式は中身から、判別できなければ拡張子から。-format pdf|pptx|xlsx|csv|visio|emf で指定も可）
...
go run ./cmd/bdf generate -font-dir fonts/ in.pptx out.bdf         # PowerPoint, Excel: フォントを探すディレクトリを追加
go run ./cmd/bdf generate -fonts system in.pptx out.bdf            # PowerPoint, Excel: フォントを埋め込まず名前で参照
