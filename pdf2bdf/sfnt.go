...
type sfnt struct {
	...
	advances   []uint16
	lsbs       []int16 // left side bearings when known (one per glyph); zero otherwise
...
// advance returns the advance width of a glyph (hmtx repeats the last one).
func (f *sfnt) advance(gid int) uint16 {
	switch {
	case gid < len(f.advances):
		return f.advances[gid]
	case len(f.advances) > 0:
		return f.advances[len(f.advances)-1]
	}
	return uint16(f.unitsPerEm / 2)
}
...
type fontInfo struct {
	...
	// font's, or Preview & Print when the original did not say (see license.go).
	fsType uint16
...
func (f *sfnt) rebuild(cmap map[uint32]uint16, info fontInfo) []byte {
