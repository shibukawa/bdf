...
type fill struct {
	...
	grad  *ooxml.Node // a:gradFill (angle, path)
	blip  *ooxml.Node // a:blipFill
...
func fillElem(n *ooxml.Node) *ooxml.Node {
	if n == nil {
		return nil
	}
...
func (s *Drawing) resolveFill(n *ooxml.Node, part string, cc *colorCtx, grp *groupCtx) fill {
	if n == nil {
		return fill{}
	}
	switch n.Name {
	...
	case "gradFill":
		...
		for _, gs := range n.Path("gsLst").Children("gs") {
			if c, ok := cc.color(gs); ok {
				f.stops = append(f.stops, gradStop{gs.AttrPct("pos", 0), c})
			}
		}
		if len(f.stops) == 0 {
			return fill{}
...
func (s *Drawing) styleFill(ref *ooxml.Node, cc *colorCtx) fill {
	if ref == nil {
		return fill{}
	}
...
func (s *Drawing) setFill(cv *canvas.Canvas, f fill, w, h float64) bool {
	switch f.kind {
	case fillSolid:
...
func gradientPaint(f fill, w, h float64) bdf.Paint {
	...
	if path := f.grad.Child("path"); path != nil {
		...
		l, t, rr, b := r.AttrPct("l", 0), r.AttrPct("t", 0), r.AttrPct("r", 0), r.AttrPct("b", 0)
...
func (c *Renderer) patternImage(f fill) bdf.Hash {
...
func (s *Drawing) resolveLine(lns []*ooxml.Node, parts []string, ref *ooxml.Node, cc *colorCtx) *line {
	themeIdx := -1
	if ref != nil {
	...
	for i, ln := range lns {
		...
		if !haveJoin {
			...
			case ln.Child("miter") != nil:
				...
				if lim := ln.Child("miter").AttrPct("lim", 0); lim > 0 {
					l.miter = lim
				}
			}
		}
		if !haveDash {
			if d := ln.Child("prstDash"); d != nil {
				...
				for _, ds := range d.Children("ds") {
					l.dash = append(l.dash, ds.AttrPct("d", 1), ds.AttrPct("sp", 1))
				}
			}
		}
		if !haveHead {
...
func parseArrow(e *ooxml.Node) arrow {
...
func setLine(cv *canvas.Canvas, l *line, w, h float64) {
...
func arrowHead(cv *canvas.Canvas, l *line, a arrow, x, y, dx, dy float64) {
	if a.kind == "" {
		return
	}
	n := math.Hypot(dx, dy)
...
func (s *Drawing) resolveShadow(effects []*ooxml.Node, ref *ooxml.Node, cc *colorCtx) *shadow {
	lst := (*ooxml.Node)(nil)
	...
	dist := sh.AttrEMU("dist", 0)
	dir := float64(sh.AttrInt("dir", 0)) * angUnit
	return &shadow{c: c, blur: sh.AttrEMU("blurRad", 0), dx: dist * math.Cos(dir), dy: dist * math.Sin(dir)}
}
