...
func parseShapeDef(n *node) *shapeDef {
	d := &shapeDef{}
...
func shapeGeometry(spPr *node, w, h float64) (*geometry, string) {
...
func evalGeometry(def *shapeDef, override *node, w, h float64) *geometry {
	e := newGuideEnv(w, h)
...
func buildPath(e *guideEnv, pn *node, w, h float64) geomPath {
	sx, sy := 1.0, 1.0
	...
	started := false
	pt := func(n *node) (float64, float64) {
