...
func blipRecolor(blip *node, cc *colorCtx) *recolor {
	rc := &recolor{}
	var key strings.Builder
	for _, k := range blip.kids() {
		...
		case "biLevel":
			fx.thresh = pct(k, "thresh", 0.5)
		case "grayscl":
		case "lum":
			b, c := pct(k, "bright", 0), pct(k, "contrast", 0)
			if b == 0 && c == 0 {
				continue
			}
...
// color applies the effects to a metafile color.
func (rc *recolor) color(c rgba) rgba {
	if rc == nil {
		return c
	}
	p := rc.pixel(color.NRGBA{uint8(to8(c.R)), uint8(to8(c.G)), uint8(to8(c.B)), uint8(to8(c.A))}, 9)
	return rgba{float64(p.R) / 255, float64(p.G) / 255, float64(p.B) / 255, float64(p.A) / 255}
}

// image applies the effects to decoded pixels in place.
func (rc *recolor) image(img *image.NRGBA, tol int) {
	if rc == nil {
		return
	}
	b := img.Bounds()
...
func (c *converter) recolored(e *imageEntry, rc *recolor) *imageEntry {
	...
	if format == "jpeg" && opaque(n) {
		...
		if err := jpeg.Encode(&buf, n, &jpeg.Options{Quality: 90}); err == nil {
			out = buf.Bytes()
			if res, err := imgconv.Optimize(out, c.opts.Images); err == nil || res.Data != nil {
				out = res.Data
			}
		}
	}
	if out == nil {
		res, _ := imgconv.EncodeImage(n, format != "jpeg", c.opts.Images)
		out = res.Data
	}
