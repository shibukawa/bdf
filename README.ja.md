...
- 読み上げ可能なテキスト層: 構造 MARK の見出し・リスト・表・代替テキスト付きの図・リンク・言語をスクリーンリーダーに伝える（タグ付き PDF と PowerPoint の構造、Excel のセルと表の見出しを変換）
...
変換は `bdf generate` サブコマンドで行い、入力の形式（PDF / PowerPoint / Excel / Visio / Windows メタファイル）は中身から判別します。
...
# PDF / PowerPoint / Excel / Visio / メタファイル → BDF（形式は中身から判別。-format pdf|pptx|xlsx|visio|emf で指定も可）
