...
func stockObject(i uint32) any {
	...
	case 1:
		return &gdiBrush{color: gray(191), hatch: -1}
	case 2:
		return &gdiBrush{color: gray(128), hatch: -1}
	case 3:
		return &gdiBrush{color: gray(64), hatch: -1}
	case 4:
...
func brushFromLog(style uint32, c color.NRGBA, hatch uint32) *gdiBrush {
	switch style {
	case 1: // BS_NULL
		return &gdiBrush{null: true}
	case 2: // BS_HATCHED
...
func patternBrush(bmi, bits []byte) *gdiBrush {
	...
	return &gdiBrush{color: color.NRGBA{uint8(math.Round(r / n)), uint8(math.Round(gg / n)), uint8(math.Round(b / n)), 255}, hatch: -1}
}

// frame is the picture box of an enhanced metafile.
type frame struct {
	box  [4]float64 // left, top, right, bottom in device pixels
	pxmm [2]float64 // device pixels per millimetre
}

// emfFrame reads the header of an enhanced metafile: its frame (the
// picture's extent in 0.01 mm on the reference device), or when that is
// empty its bounds.
func emfFrame(b []byte) (f frame, ok bool) {
	if len(b) < 88 {
		return f, false
	}
	...
	f.pxmm = [2]float64{96 / 25.4, 96 / 25.4}
	if devX > 0 && mmX > 0 && devY > 0 && mmY > 0 {
		f.pxmm = [2]float64{devX / mmX, devY / mmY}
	}
	f.box = [4]float64{fl / 100 * f.pxmm[0], ft / 100 * f.pxmm[1], fr / 100 * f.pxmm[0], fb / 100 * f.pxmm[1]}
	if f.box[2]-f.box[0] <= 0 || f.box[3]-f.box[1] <= 0 {
		f.box = [4]float64{bl, bt, br + 1, bb + 1}
	}
	return f, f.box[2]-f.box[0] > 0 && f.box[3]-f.box[1] > 0
}
...
func (g *gdi) playEMF(b []byte, x, y, w, h float64) {
	...
	g.pxmm = f.pxmm[0]
	x0, y0, x1, y1 := f.box[0], f.box[1], f.box[2], f.box[3]
	g.out = canvas.Translate(x, y).Mul(canvas.Scale(w/(x1-x0), h/(y1-y0))).Mul(canvas.Translate(-x0, -y0))
	pathFigure := false
	for p := 0; p+8 <= len(b); {
		...
		case 35: // SETWORLDTRANSFORM
			g.st.world = canvas.Matrix{lef32(r, 8), lef32(r, 12), lef32(r, 16), lef32(r, 20), lef32(r, 24), lef32(r, 28)}
		case 36: // MODIFYWORLDTRANSFORM
			x := canvas.Matrix{lef32(r, 8), lef32(r, 12), lef32(r, 16), lef32(r, 20), lef32(r, 24), lef32(r, 28)}
			switch le32(r, 32) {
			case 1:
				g.st.world = canvas.Identity
			case 2:
		...
		case 83, 84: // EXTTEXTOUTA, EXTTEXTOUTW
			...
			if opts&0x10 != 0 { // ETO_GLYPH_INDEX
				g.warn("metafile text drawn by glyph index is not supported")
				continue
			}
			if n <= 0 || offStr >= len(r) {
				continue
		...
		case 70: // GDICOMMENT: EMF+ records are skipped
			if len(r) >= 20 && string(r[12:16]) == "EMF+" && le16(r, 16) == 0x4001 && le16(r, 18)&1 == 0 {
				...
				g.warn("EMF+ only metafiles are not supported")
			}
		}
	}
}
